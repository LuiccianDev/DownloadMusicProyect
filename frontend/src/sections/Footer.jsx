import React from "react";
import { Link } from "react-router-dom";

export default function Footer() {
    return (
        <div className="items-center justify-center p-5 bg-gray-100 dark:bg-gray-950 pt-35">
            <div className="text-gray-800 dark:text-white gap-x-6 section">
                <div className="w-full text-2xl lg:text-xl font-bold">
                    <h1 className="w-full">How can we help you. get in touch</h1>
                </div>
                <div className="flex mt-8 flex-col md:flex-row md:justify-between">
                    <p className="w-full md:w-2/3 text-gray-600 dark:text-gray-400">
                        Do you have any questions about the download? Contact us, and we will be happy
                        to help ensure a smooth experience.
                    </p>
                    <div className="w-44 pt-6 md:pt-0">
                        <button className="cursor-pointer">
                            <a href="/Contact" className="transform transition-transform duration-300 hover:scale-110 bg-gradient-to-r from-purple-600 to-pink-600 justify-center text-center rounded-lg shadow px-10 py-3 flex items-center text-white">
                                Contact US
                            </a>
                        </button>
                    </div>
                </div>
                <div className="flex flex-col">
                    <div className="flex mt-15 mb-10 flex-row justify-between items-center">
                        <div className="font-Avatar text-3xl">
                            Lazdev
                        </div>
                        <Link to="/About" className="hidden md:block cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white uppercase">
                            About
                        </Link>
                        <Link to="/Why-us" className="hidden md:block cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white uppercase">
                            Why us
                        </Link>
                        <Link to="/Contact" className="hidden md:block cursor-pointer text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white uppercase">
                            Contact
                        </Link>
                        <div className="flex flex-row space-x-8 items-center justify-between">
                            <a className="cursor-pointer transform transition-transform duration-300 hover:scale-110" href="#">
                                <svg width="18" height="18" viewBox="0 0 6 12" className="fill-current text-gray-800 dark:text-white">
                                    <path d="M3.89782 12V6.53514H5.67481L5.93895 4.39547H3.89782V3.03259C3.89782 2.41516 4.06363 1.99243 4.91774 1.99243H6V0.0847928C5.47342 0.0262443 4.94412 -0.00202566 4.41453 0.000112795C2.84383 0.000112795 1.76542 0.994936 1.76542 2.82122V4.39147H0V6.53114H1.76928V12H3.89782Z" />
                                </svg>
                            </a>
                            <a className="cursor-pointer transform transition-transform duration-300 hover:scale-110" href="#">
                                <svg width="18" height="18" viewBox="0 0 12 13" className="fill-current text-gray-800 dark:text-white">
                                    <path d="M5.99536 2.91345C5.17815 2.91345 4.39441 3.23809 3.81655 3.81594C3.2387 4.3938 2.91406 5.17754 2.91406 5.99475C2.91406 6.81196 3.2387 7.5957 3.81655 8.17356C4.39441 8.75141 5.17815 9.07605 5.99536 9.07605C6.81257 9.07605 7.59631 8.75141 8.17417 8.17356C8.75202 7.5957 9.07666 6.81196 9.07666 5.99475C9.07666 5.17754 8.75202 4.3938 8.17417 3.81594C7.59631 3.23809 6.81257 2.91345 5.99536 2.91345ZM5.99536 7.99586C5.46446 7.99586 4.9553 7.78496 4.57989 7.40955C4.20448 7.03415 3.99358 6.52499 3.99358 5.99408C3.99358 5.46318 4.20448 4.95402 4.57989 4.57861C4.9553 4.20321 5.46446 3.99231 5.99536 3.99231C6.52626 3.99231 7.03542 4.20321 7.41083 4.57861C7.78624 4.95402 7.99714 5.46318 7.99714 5.99408C7.99714 6.52499 7.78624 7.03415 7.41083 7.40955C7.03542 7.78496 6.52626 7.99586 5.99536 7.99586Z" />
                                    <path d="M9.19863 3.51848C9.59537 3.51848 9.91698 3.19687 9.91698 2.80013C9.91698 2.4034 9.59537 2.08179 9.19863 2.08179C8.8019 2.08179 8.48029 2.4034 8.48029 2.80013C8.48029 3.19687 8.8019 3.51848 9.19863 3.51848Z" />
                                    <path d="M11.6821 2.06975C11.5279 1.67138 11.2921 1.30961 10.99 1.00759C10.6879 0.705576 10.326 0.469972 9.92759 0.31586C9.46135 0.140842 8.9688 0.0462069 8.4709 0.0359839C7.82919 0.00799638 7.62594 0 5.99867 0C4.37139 0 4.16282 -6.70254e-08 3.52643 0.0359839C3.02891 0.0456842 2.53671 0.140339 2.07108 0.31586C1.67255 0.469792 1.31059 0.705333 1.00844 1.00737C0.706289 1.30941 0.47061 1.67127 0.316526 2.06975C0.141474 2.53595 0.0470554 3.02855 0.0373167 3.52643C0.00866281 4.16748 0 4.37072 0 5.99867C0 7.62594 -4.96485e-09 7.83319 0.0373167 8.4709C0.0473123 8.96935 0.14127 9.46113 0.316526 9.92825C0.471042 10.3266 0.70695 10.6883 1.00918 10.9903C1.3114 11.2923 1.6733 11.5279 2.07175 11.6821C2.5365 11.8642 3.0289 11.9656 3.52777 11.982C4.16948 12.01 4.37272 12.0187 6 12.0187C7.62728 12.0187 7.83585 12.0187 8.47223 11.982C8.97008 11.9719 9.46262 11.8775 9.92892 11.7028C10.3272 11.5483 10.689 11.3125 10.9911 11.0104C11.2932 10.7083 11.529 10.3466 11.6835 9.94825C11.8587 9.48179 11.9527 8.99 11.9627 8.49156C11.9913 7.85051 12 7.64727 12 6.01932C12 4.39138 12 4.18481 11.9627 3.54709C11.9549 3.04216 11.86 2.54237 11.6821 2.06975ZM10.8705 8.42159C10.8662 8.80562 10.7961 9.18608 10.6633 9.54642C10.5632 9.80555 10.41 10.0409 10.2135 10.2372C10.017 10.4336 9.78162 10.5867 9.52243 10.6866C9.16608 10.8188 8.78967 10.8889 8.4096 10.8938C7.77654 10.9231 7.59796 10.9305 5.97468 10.9305C4.35007 10.9305 4.18414 10.9305 3.53909 10.8938C3.15921 10.8892 2.78298 10.8191 2.42692 10.6866C2.16683 10.5873 1.93048 10.4345 1.73316 10.2381C1.53584 10.0417 1.38194 9.80605 1.28143 9.54642C1.15045 9.18995 1.08039 8.81398 1.07419 8.43425C1.04554 7.8012 1.03887 7.62261 1.03887 5.99933C1.03887 4.37539 1.03887 4.20946 1.07419 3.56375C1.0785 3.17993 1.14859 2.7997 1.28143 2.43958C1.48467 1.91382 1.90116 1.5 2.42692 1.29876C2.78316 1.16691 3.15928 1.09682 3.53909 1.09151C4.17281 1.06286 4.35073 1.05486 5.97468 1.05486C7.59862 1.05486 7.76522 1.05486 8.4096 1.09151C8.7897 10.8892 8.78967 10.8191 9.16608 10.6866C9.52243 10.5867 9.78162 10.4336 10.017 10.2372C10.2135 10.0409 10.41 9.80555 10.5632 9.54642C10.6633 9.18608 10.7961 8.80562 10.8662 8.42159C10.9231 7.77654 10.9305 7.59796 10.9305 5.97468C10.9305 4.35007 10.9305 4.18414 10.8938 3.53909C10.8892 3.15921 10.8191 2.78298 10.6866 2.42692C10.5873 2.16683 10.4345 1.93048 10.2381 1.73316C10.0417 1.53584 9.80605 1.38194 9.54642 1.28143C9.18995 1.15045 8.81398 1.08039 8.43425 1.07419C7.8012 1.04554 7.62261 1.03887 5.99933 1.03887C4.37539 1.03887 4.20946 1.03887 3.56375 1.07419C3.17993 1.0785 2.7997 1.14859 2.43958 1.28143C1.91382 1.48467 1.5 1.90116 1.29876 2.42692C1.16691 2.78316 1.09682 3.15928 1.09151 3.53909C1.06286 4.17281 1.05486 4.35073 1.05486 5.97468C1.05486 7.59862 1.05486 7.76522 1.09151 8.4096C10.8892 8.7897 10.8191 8.78967 10.6866 9.16608C10.5873 9.52243 10.4345 9.78162 10.2381 10.017C10.0417 10.2135 9.80605 10.41 9.54642 10.5632C9.18995 10.6633 8.81398 10.7961 8.43425 10.8662C7.8012 10.9231 7.62261 10.9305 5.99933 10.9305C4.37539 10.9305 4.20946 10.9305 3.56375 10.8938C3.17993 10.8892 2.7997 10.8191 2.43958 10.6866C1.91382 10.4847 1.5 10.0988 1.29876 9.57308C1.16691 9.21684 1.09682 8.84072 1.09151 8.46091C1.06286 7.82719 1.05486 7.64927 1.05486 6.02532C1.05486 4.40138 1.06286 4.22346 1.09151 3.58909Z" />
                                </svg>
                            </a>
                            <a className="cursor-pointer transform transition-transform duration-300 hover:scale-110" href="#">
                                <svg width="18" height="18" viewBox="0 0 13 9" className="fill-current text-gray-800 dark:text-white">
                                    <path d="M12.7355 1.415C12.6616 1.14357 12.517 0.896024 12.3162 0.697014C12.1154 0.498004 11.8654 0.354468 11.5911 0.280692C10.5739 0.00450089 6.5045 4.87928e-06 6.5045 4.87928e-06C6.5045 4.87928e-06 2.43578 -0.00449139 1.41795 0.259496C1.14379 0.336667 0.894302 0.482233 0.693428 0.68222C0.492554 0.882207 0.347041 1.1299 0.270859 1.40152C0.00259923 2.40737 9.51671e-07 4.49358 9.51671e-07 4.49358C9.51671e-07 4.49358 -0.0025972 6.59006 0.263714 7.58564C0.413109 8.13609 0.851549 8.57094 1.40885 8.71931C2.43643 8.9955 6.49476 9 6.49476 9C6.49476 9 10.5641 9.00449 11.5813 8.74115C11.8557 8.6675 12.106 8.52429 12.3073 8.32569C12.5086 8.12709 12.6539 7.87996 12.729 7.60876C12.998 6.60355 12.9999 4.51798 12.9999 4.51798C12.9999 4.51798 13.0129 2.42086 12.7355 1.415ZM5.20282 6.42628L5.20607 2.57244L8.58823 4.50257L5.20282 6.42628Z" />
                                </svg>
                            </a>
                            <a className="cursor-pointer transform transition-transform duration-300 hover:scale-110" href="#">
                                <svg width="18" height="18" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" className="fill-current text-gray-800 dark:text-white">
                                    <path d="M7.976 0A7.977 7.977 0 0 0 0 7.976c0 3.522 2.3 6.507 5.431 7.584.392.049.538-.196.538-.392v-1.37c-2.201.49-2.69-1.076-2.69-1.076-.343-.93-.881-1.175-.881-1.175-.734-.489.048-.489.048-.489.783.049 1.224.832 1.224.832.734 1.223 1.859.88 2.3.685.048-.538.293-.88.489-1.076-1.762-.196-3.621-.881-3.621-3.964 0-.88.293-1.566.832-2.153-.05-.147-.343-.978.098-2.055 0 0 .685-.196 2.201.832.636-.196 1.322-.245 2.007-.245s1.37.098 2.006.245c1.517-1.027 2.202-.832 2.202-.832.44 1.077.146 1.908.097 2.104a3.16 3.16 0 0 1 .832 2.153c0 3.083-1.86 3.719-3.62 3.915.293.244.538.733.538 1.467v2.202c0 .196.146.44.538.392A7.984 7.984 0 0 0 16 7.976C15.951 3.572 12.38 0 7.976 0z" />
                                </svg>
                            </a>
                        </div>
                    </div>
                    <hr className="border-gray-300 dark:border-gray-600" />
                    <p className="w-full text-center my-5 text-gray-500 dark:text-gray-600">
                        Copyright © 2025 LuccianDev
                    </p>
                </div>
            </div>
        </div>
    );
}